...
@WebServlet(urlPatterns= {"/UpdateEmailStudent"})
public class UpdateEmailStudent extends HttpServlet {
	@Override
...
//			String query = "select email from educator where email=?";
//			boolean flag = true;
//			
...
//			}
			...
			String query =  "select email from student where email=? and std_id != ?";
			...
			query =  "select validate_email from student where std_id=?";
			...
			if(flag == false) {
				...
				//To check if this student is already active or not for concurrency control
				
				session.setAttribute("exist2", "email");
				resp.sendRedirect("check_students.jsp");
				
			}else{
				flag = new ValidateStudent().validateEmail(email);
				...
				query = "update student set email=?,date_of_linkGen=now() where std_id=?";
				ps= conn.prepareStatement(query);
				...
				if(ps.executeUpdate() > 0) {
					...
					String upId = null;
					//do encoding of ids
					encodeId= hd.encodeMethod(""+std_id);
			...
			System.out.println("Exception at UpdateEmailStudent : "+e);
