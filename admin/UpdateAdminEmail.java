...
@WebServlet(urlPatterns = { "/UpdateAdminEmail" })
...
//			}
			...
			if (flag == false) {
				...
				query = "update admin set email=?,validate_email=?,date_of_linkGen=now() where admin_id=?";
				ps = conn.prepareStatement(query);
				...
				if (ps.executeUpdate() > 0) {
					...
					encodeId = hd.encodeMethod("" + admin_id);
					query = "select date_of_linkGen from admin where admin_id=?";
					ps = conn.prepareStatement(query);
					...
					query = "update admin set link=? where admin_id=?";

					ps = conn.prepareStatement(query);
					ps.setString(1,
							"http://localhost:8081/EduHITec/confirmAdminEmail.jsp?id=" + encodeId + "&upid=" + upId);
					ps.setInt(2, admin_id);
					...
					if (ps.executeUpdate() > 0) {
						...
						msg.setContent("<div>\r\n" + "        <b>Request To Update Email</b>,<br><br>\r\n"
								...
								+ "        <a href=\"http://localhost:8081/EduHITec/confirmAdminEmail.jsp?id=" + encodeId
								+ "&upid=" + upId + "\">" + "http://localhost:8081/EduHITec/confirmAdminEmail.jsp?id="
			...
			System.out.println("Exception at UpdateEmail :"+e);
			e.printStackTrace();
