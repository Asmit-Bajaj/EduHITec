...
@WebServlet(urlPatterns= {"/UpdateStudentProfileEmail"})
public class UpdateStudentProfileEmail extends HttpServlet{
	@Override
...
//			// check in educator table too for email
...
//			}
			...
			if (flag == false) {
				...
				query = "update student set email=?,validate_email=?,date_of_linkGen=now() where std_id=?";
				ps = conn.prepareStatement(query);
				...
				if (ps.executeUpdate() > 0) {
					...
					ps = conn.prepareStatement(query);
					ps.setString(1,
							"http://localhost:8081/EduHITec/confirmStudentMail.jsp?id=" + encodeId + "&upid=" + upId);
					ps.setInt(2, std_id);
					...
					if (ps.executeUpdate() > 0) {
						...
						msg.setContent("<div>\r\n" + "        <b>Request To Update Email</b>,<br><br>\r\n"
								...
								+ "        <a href=\"http://localhost:8081/EduHITec/confirmStudentMail.jsp?id=" + encodeId
								+ "&upid=" + upId + "\">" + "http://localhost:8081/EduHITec/confirmStudentMail.jsp?id="
			...
			System.out.println("Exception at UpdateStudentProfileEmail :"+e);
			e.printStackTrace();
